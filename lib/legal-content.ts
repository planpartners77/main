...
export const LEGAL_DOCS: Record<LegalDocSlug, LegalDoc> = {
  terms: {
    ...
    sections: [
      ...
      {
        heading: "제3조의2 (간편가입을 통한 회원가입)",
        body: [
          "이용자는 이메일 가입 외에도 카카오 등 회사가 정한 제휴 플랫폼 계정을 이용한 간편가입 방식으로 회원가입을 할 수 있습니다.",
          "간편가입 시 회사는 이용자가 해당 플랫폼에서 동의한 범위 내의 정보(이름, 성별, 생년월일, 휴대폰번호 등)를 제공받아 회원 식별 및 서비스 제공 목적으로 이용하며, 구체적인 수집 항목과 처리 방법은 개인정보처리방침에 따릅니다.",
          "간편가입으로 가입한 이용자는 마이페이지를 통해 해당 플랫폼 계정과의 연동을 해제할 수 있으며, 연동 해제 후에는 서비스 재이용을 위해 별도의 본인확인 절차가 필요할 수 있습니다.",
          "회사는 제휴 플랫폼 사업자의 시스템 장애, 정책 변경, 서비스 종료 등 회사의 귀책사유가 아닌 사유로 간편가입·로그인이 일시적으로 제한되는 것에 대해 책임을 지지 않습니다.",
        ],
      },
      {
        heading: "제4조 (서비스 이용)",
    ...
    sections: [
      {
        ...
        body: [
          "회원가입 시(이메일 가입): 이메일, 비밀번호, 이름, 휴대폰번호, 성별, 생년월일",
          "회원가입 시(카카오 간편가입): 카카오계정과 연동된 이름, 성별, 생년월일(생일·출생연도), 휴대폰번호, 연계정보(CI). CI는 원본을 저장하지 않고 별도 값을 추가해 복원 불가능한 형태로 암호화(해시)하여 저장합니다.",
          "상품 신청·상담 접수 시: 이름, 연락처, 주소 등 신청서에 기재한 정보",
          "이벤트·경품 등 배송이 필요한 경우(선택 동의 시): 수령인명, 배송지 주소, 연락처",
